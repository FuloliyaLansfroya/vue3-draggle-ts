...
export const resetFun = (
    ...
    if (inerCol.clientWidth > unitLength || inerCol.clientHeight > unitLength) {
        if (
            inerCol.clientWidth > unitLength &&
            inerCol.clientHeight > unitLength
        ) {
            ...
            if (draggleDirection === 'right') {
                for (let j = 0; j < leftYPlus; j++) {
                    if (
                        ...
                        logCube[topX + i][leftY + j] &&
                        logCube[topX + i][leftY + j].value === 1 &&
                        logCube[topX + i][leftY + j].label !== type
                    ) {
        ...
        for (let i = 0; i < topXPlus; i++) {
            for (let j = 0; j < leftYPlus; j++) {
                ...
                if (type === 'left') {
                    if (
                        (logCube[topX + i] &&
                            logCube[topX + i][leftY - j - 1] &&
                            logCube[topX + i][leftY - j - 1].value !== 0) ||
                        leftY - j - 1 < 0
                    ) {
                        return [false];
                    }
